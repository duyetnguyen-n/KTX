...
if (empty($_SESSION['username'])) {
    ...
    $sql = "SELECT * FROM tang WHERE id = $id";
    $result = $db->select($sql);
    if ($result->num_rows == 1) {
        $row = $result->fetch_assoc();
        $ten_tang = $row["ten_tang"];
        $khuvuc= $row["khuvuc"];
        $ngaytao = $row["ngaytao"];
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $newten_tang = $_POST["ten_tang"];
        $newkhuvuc= $_POST["khuvuc"];
        ...
        $sql = "UPDATE tang SET 
            ten_tang='$newten_tang',
            khuvuc='$newkhuvuc',
            anh='$newanh'
            WHERE id='$id'";

        if ($db->update($sql)) {
            echo '<script>alert("Sửa tầng thành công!"); window.location.href = "list-tang.php";</script>';
            exit(); // Đảm bảo dừng thực thi mã nguồn sau khi chuyển hướng
        } else {
            echo '<script>alert("Lỗi khi sửa tầng ' . $db->getError() . '");</script>';
        }
    } ?>
        ...
        <div class="col-12 container-content">
            <div class="header-content pb-3">
                <div class="title-content">
                    <h3 class="m-0 strong">Sửa tầng</h3>
                    <div class="span">Quản lý tầng của bạn</div>
                </div>
            </div>
            <div class="content-add-room col-12">
                <form action="edit-tang.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-12 col-md-6 p-2">
                            <label for="ten_tang">Tên tầng</label>
                            <input type="text" id="ten_tang" name="ten_tang" class="form-control" value="<?php echo $ten_tang; ?>" required>
                        </div>
                        <div class="col-12 col-md-6 p-2">
                            <label for="khuvuc">Khu vực</label>
                            <input type="text" id="khuvuc" name="khuvuc" class="form-control" value="<?php echo $khuvuc; ?>" required>
                        </div>
